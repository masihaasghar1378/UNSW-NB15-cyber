...
for method in methods:
    start = time.time()
    print("="*80)
    print(f" STARTING Hybrid CNN + {method.upper()} at {time.strftime('%H:%M:%S')}")
    ...
    # Reshape for CNN
    X_train_cnn = X_train_sel.reshape((X_train_sel.shape[0], n_selected, 1))
    X_test_cnn = X_test_sel.reshape((X_test_sel.shape[0], n_selected, 1))
    
    # Stage 1: Binary Detection
    model_binary = Sequential([
        Conv1D(128, 3, activation='relu', input_shape=(n_selected, 1), padding='same'),
        MaxPooling1D(2),
    ...
    model_binary.fit(X_train_cnn, y_train_binary, epochs=20, batch_size=2048, verbose=0, validation_split=0.1)
    probs_binary = model_binary.predict(X_test_cnn, batch_size=2048, verbose=0).ravel()
    ...
    if np.any(attack_mask_test):
        X_train_attacks_cnn = X_train_cnn[attack_mask_train]
        X_test_attacks_cnn = X_test_cnn[attack_mask_test]
        ...
        model_multi.fit(X_train_attacks_cnn, y_train_attacks, epochs=20, batch_size=2048, verbose=0, validation_split=0.1)
        probs_multi = model_multi.predict(X_test_attacks_cnn, batch_size=2048, verbose=0)
        ...
        # XAI: SHAP
        background = X_train_attacks_cnn[np.random.choice(X_train_attacks_cnn.shape[0], 100, replace=False)]
        explainer = shap.GradientExplainer(model_multi, background)
        shap_values = explainer.shap_values(X_test_attacks_cnn[:1000])
...
pd.DataFrame(metrics_results).sort_values("Multi_AUC_OVR", ascending=False).round(5).to_csv(
    r"D:\PHD WORK\research for thesis\metrics_hybrid_cnn.csv", index=False)
pd.DataFrame(roc_results).to_csv(
    r"D:\PHD WORK\research for thesis\roc_curves_hybrid_cnn.csv", index=False)
pd.DataFrame(shap_results).to_csv(
    r"D:\PHD WORK\research for thesis\shap_summaries_hybrid_cnn.csv", index=False)
print("\n" + "="*60)
print(" Hybrid CNN Benchmark Completed!")
print("="*60)
