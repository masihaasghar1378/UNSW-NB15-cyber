...
]  # Last two are labels, but features are first 43? Wait, UNSW has 42 feats + id? Adjust to 0-46 as per scripts.

# Actually, UNSW-NB15 has 42 features + id (43) but scripts treat :-2 as features (47 cols? Standard is 49 total: 1 id + 42 feats + attack_cat + label = 44? Clarify from data.
# Standard: 49 cols, but no id in some, assume 0-46 feats for safety. User has header=None, so use f'feat_{i}' for simplicity.
feature_names = [f'feat_{i}' for i in range(47)]  # Safe, customize if needed.
...
for method in methods:
    start = time.time()
    print("="*80)
    print(f" STARTING Hybrid XGBoost + {method.upper()} at {time.strftime('%H:%M:%S')}")
    ...
    model_binary.fit(X_train_sel, y_train_binary)
    probs_binary = model_binary.predict_proba(X_test_sel)[:, 1]
    ...
    if np.any(attack_mask_test):
        ...
        model_multi.fit(X_train_attacks, y_train_attacks)
        X_test_attacks = X_test_sel[attack_mask_test]
        probs_multi = model_multi.predict_proba(X_test_attacks)
        ...
        shap_values = explainer.shap_values(X_test_attacks[:1000])
...
pd.DataFrame(metrics_results).sort_values("Multi_AUC_OVR", ascending=False).round(5).to_csv(
    r"D:\PHD WORK\research for thesis\metrics_hybrid_xgboost.csv", index=False)
pd.DataFrame(roc_results).to_csv(
    r"D:\PHD WORK\research for thesis\roc_curves_hybrid_xgboost.csv", index=False)
pd.DataFrame(shap_results).to_csv(
    r"D:\PHD WORK\research for thesis\shap_summaries_hybrid_xgboost.csv", index=False)
print("\n" + "="*60)
print(" Hybrid XGBoost Benchmark Completed!")
print("="*60)