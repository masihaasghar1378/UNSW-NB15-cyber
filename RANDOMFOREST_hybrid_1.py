...
for method in methods:
    start = time.time()
    print("="*80)
    print(f" STARTING Hybrid RandomForest + {method.upper()} at {time.strftime('%H:%M:%S')}")
...
pd.DataFrame(metrics_results).sort_values("Multi_AUC_OVR", ascending=False).round(5).to_csv(
    r"D:\PHD WORK\research for thesis\metrics_hybrid_randomforest.csv", index=False)
pd.DataFrame(roc_results).to_csv(
    r"D:\PHD WORK\research for thesis\roc_curves_hybrid_randomforest.csv", index=False)
pd.DataFrame(shap_results).to_csv(
    r"D:\PHD WORK\research for thesis\shap_summaries_hybrid_randomforest.csv", index=False)
print("\n" + "="*60)
print(" Hybrid RandomForest Benchmark Completed!")
print("="*60)